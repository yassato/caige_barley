...
# load GWAS results
trait = "SFNB" # select the trait name: NFNB, SNFB, or Scald
...
out = read.csv("./output/PVE_NFNB_MAF5.csv")
...
bar1 = ggplot(data=res1,aes(x=dist,y=PVE,fill=type)) + geom_bar(stat="identity") + theme_classic() + 
  ylab("PVE") + xlab("Euclidean distance from focal plants") + 
  ylim(NA,1) + theme(legend.position=c(0.8,0.9)) #+ labs(subtitle="Net form net blotch")
  
PVE12 = c(out$PVEself,out$PVEnei)
res12 = data.frame(dist,PVE12,type)
bar12 = ggplot(data=res12,aes(x=dist,y=PVE12,fill=type)) + geom_bar(stat="identity") + theme_classic() + 
  ylab("PVE") + xlab("Euclidean distance from focal plants") + 
  ylim(NA,1) + theme(legend.position=c(0.8,0.9)) + labs(subtitle="(A) Net form net blotch")


out = read.csv("./output/PVE_SFNB_MAF5.csv")
...
PVE22 = c(out$PVEself,out$PVEnei)
res22 = data.frame(dist,PVE22,type)
bar22 = ggplot(data=res22,aes(x=dist,y=PVE22,fill=type)) + geom_bar(stat="identity") + theme_classic() + 
  ylab("PVE") + xlab("Euclidean distance from focal plants") + 
  ylim(NA,1) + theme(legend.position=c(0.8,0.9)) + labs(subtitle="(B) Spot form net blotch")
...
PVE32 = c(out$PVEself,out$PVEnei)
res32 = data.frame(dist,PVE32,type)
bar32 = ggplot(data=res32,aes(x=dist,y=PVE32,fill=type)) + geom_bar(stat="identity") + theme_classic() + 
  ylab("PVE") + xlab("Euclidean distance from focal plants") + 
  ylim(NA,1) + theme(legend.position=c(0.8,0.9)) + labs(subtitle="(C) Scald")
...
ggsave(bar,filename="./output/PVE_all_maf5.pdf",height=6,width=6)

bar2 = (bar12 | bar22 | bar32) #+ plot_annotation(tag_levels = "A")
ggsave(bar2,filename="./output/PVE_part_maf5.pdf",height=4,width=10)
